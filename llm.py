...
- **프로젝트 정보:** 각 항목은 순서대로 아래 필드로 구성된다.
    ...
    - p_name: 프로젝트 제목
    - p_content: 프로젝트 설명
    - p_method: 프로젝트 개발 방식, 각 번호가 의미하는 바는 다음과 같다: [0:폭포수, 1:애자일, 2:기타]
    - p_memcount: 참여 인원 수
    - p_start: 프로젝트 시작일
    - p_end: 프로젝트 종료일
    - s.subj_name: 과목명  
...
PROMPTS = [
    ...
    프로젝트 설명과 개발 방식을 바탕으로 다음 내용을 중심으로 간략하게 조언해줘:

    1. 프로젝트 주제에 적합한 WBS 항목을 예시로 제시해줘.
    2. 앞으로 어떤 온라인 산출물을 어떤 순서와 방식으로 작성하면 좋을지 제안해줘.
    3. 팀원은 자신을 포함해 최대 4명까지만 가능하므로, 효율적인 역할 분배 방법을 알려줘.
    4. PMS를 어떤 방향으로 활용하면 프로젝트를 체계적으로 관리할 수 있을지 간단히 안내해줘.
